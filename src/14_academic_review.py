# -*- coding: utf-8 -*-
"""
14_academic_review.py
Writes an academic critical review of the methodology and saves it
as a markdown report + a workbook sheet.

Outputs:
  output/final/academic_review.md
  output/final/Sultanbeyli_Final_Results.xlsx  (extended with 'Academic_Review' sheet)
"""
...
review_md = """# Akademik Kritik Degerlendirme - Sultanbeyli Konteyner Optimizasyonu
...
(FINAL / "academic_review.md").write_text(review_md, encoding="utf-8")
print(f"[OK] {FINAL / 'academic_review.md'}  ({len(review_md)} chars)")

# ---------------------------------------------------------------------------
# Add 'Academic_Review' sheet to the consolidated xlsx
# ---------------------------------------------------------------------------
WB = FINAL / "Sultanbeyli_Final_Results.xlsx"
wb = load_workbook(WB)
if "Academic_Review" in wb.sheetnames:
    del wb["Academic_Review"]
ws = wb.create_sheet("Academic_Review")

HDR = PatternFill(start_color="1F4E78", end_color="1F4E78", fill_type="solid")
WHITE = Font(color="FFFFFF", bold=True)
BOLD = Font(bold=True)
SUB = PatternFill(start_color="D9E1F2", end_color="D9E1F2", fill_type="solid")
LEFT = Alignment(horizontal="left", vertical="top", wrap_text=True)
CENTER = Alignment(horizontal="center", vertical="center", wrap_text=True)
THIN = Side(border_style="thin", color="BFBFBF")
BOX = Border(left=THIN, right=THIN, top=THIN, bottom=THIN)

# Parse the review_md into sections and write them
sections = []
current = None
for line in review_md.split("\n"):
    if line.startswith("# ") and not line.startswith("## "):
        ...
        current = {"title": line[2:].strip(), "level": 1, "rows": []}
    elif line.startswith("## "):
        if current:
            sections.append(current)
        current = {"title": line[3:].strip(), "level": 2, "rows": []}
    elif line.startswith("### "):
        if current:
            sections.append(current)
        current = {"title": line[4:].strip(), "level": 3, "rows": []}
...
for sec in sections:
    # Title
    cell = ws.cell(row=row, column=1, value=sec["title"])
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=4)
    if sec["level"] == 1:
        cell.fill = HDR; cell.font = WHITE; cell.font = Font(bold=True, color="FFFFFF", size=14)
    elif sec["level"] == 2:
        cell.fill = SUB; cell.font = BOLD
    else:
        cell.font = BOLD
    cell.alignment = LEFT
    row += 1
...
# Move the sheet to the end (after README)
wb.move_sheet("Academic_Review", offset=len(wb.sheetnames) - wb.sheetnames.index("Academic_Review") - 1)
wb.save(WB)
print(f"[OK] Added 'Academic_Review' sheet to {WB.name}")
