"""
05_map.py - CA9c secim haritalari
"""
import os
import pandas as pd
import numpy as np
...
for tag in ["Baseline_MEV_hard", "Baseline_NMEV_hard"]:
    ...
    ax.set_title(f"CA9c - {tag} (spatial: her mahallede >= 1)")
    ax.set_xlabel("Boylam"); ax.set_ylabel("Enlem")
    ax.legend(loc="lower right")
    ax.grid(alpha=0.3)
    plt.tight_layout()
    out = os.path.join(MAPS_DIR, f"selection_map_CA9c_{tag}.png")
...
ax.set_title("CA9c - Z ve RxC (6 hard senaryo)")
