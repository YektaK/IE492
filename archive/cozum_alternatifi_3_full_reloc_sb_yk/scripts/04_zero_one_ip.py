...
==========================================================
Ciktilar:
  - results/selection_<scenario>_<mode>_CA3.xlsx
  - results/selection_sensitivity_CA3.xlsx
  - results/p_access_per_site_CA3.xlsx
"""
import unicodedata
import numpy as np
import pandas as pd
...
def load_inputs():
    mu_pool = pd.read_excel(RES / "mu_pool_CA3.xlsx")
    mu_mev_zero = pd.read_excel(RES / "mu_mevcut_zero_CA3.xlsx")
    ...
    crit = pd.read_excel(RES / "topsis_sonuclar_CA3.xlsx")
...
def solve_ip(mu_pool_df, mu_mev_df, mahalleler, risk_map, p_access_arr,
    ...
    n = len(mu_pool_df)
    prob = pulp.LpProblem(f"CA3_Sultanbeyli_{scenario_name}_{mode}", pulp.LpMaximize)
...
if __name__ == "__main__":
    print("=" * 60)
    print(f"STEP 4: 0-1 IP - Full Relocation (20 secim) - CA3 (SB+YK)")
    print("=" * 60)
    ...
    p_access_df.to_excel(RES / "p_access_per_site_CA3.xlsx", index=False)
    print(f"[OK] p_access_per_site_CA3.xlsx")
    ...
    for sc in ["Baseline", "DamageFocused", "InfrastructureFocused"]:
        for mode in ["hard", "soft"]:
            ...
            out_path = RES / f"selection_{sc}_{mode}_CA3.xlsx"
    ...
    df_res.to_excel(RES / "selection_sensitivity_CA3.xlsx", index=False)
    print(f"\n[OK] selection_sensitivity_CA3.xlsx")
    print()
    print("--- CA3 Sensitivity Summary ---")
