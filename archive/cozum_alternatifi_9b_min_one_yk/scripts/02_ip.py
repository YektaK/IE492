"""
02_ip.py - CA9b: min 1 spatial + yol kapanma (P_access)
- 12 senaryo (3 AHP x 2 mod x hard/soft)
- mu(i,j) *= P_access(j_mahalle) per site
"""
import os, json
import numpy as np
import pandas as pd
...
p_access = np.load(os.path.join(DATA_DIR, "p_access_vec.npy"))
...
for i, m in enumerate(mahalle_cols):
    mu_a[mahalle_cols[i]] = mu_a[mahalle_cols[i]] * p_access[i]
    mu_m_sub[mahalle_cols[i]] = mu_m_sub[mahalle_cols[i]] * p_access[i]

risk_mah_col = "mahalle" if "mahalle" in risk.columns else next(c for c in risk.columns if "mahalle" in c.lower())
risk_R = risk.set_index(risk_mah_col)["risk_score"].apply(lambda v: float(v) if pd.notna(v) else 0.0)
...
for ahp_name in AHP_SCEN:
    ...
    for mode in ["MEV", "NMEV"]:
        for soft in [False, True]:
            ...
            n_total = 20
            prob = pulp.LpProblem(f"CA9b_{tag}", pulp.LpMaximize)
