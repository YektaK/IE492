"""
04_final_xlsx.py - CA9a 18-sayfa konsolide xlsx
"""
import os
import pandas as pd
...
OUT = os.path.join(FINAL_DIR, "Sultanbeyli_Final_Results_CA9a.xlsx")
...
with pd.ExcelWriter(OUT, engine="openpyxl") as w:
    pd.DataFrame({
        ...
        "value": ["CA9a (min 1 konteyner per mahalle - SPATIAL)",
    ...
    pd.read_excel(os.path.join(ROOT, CA, "comparison", "CA9a_vs_baseline.xlsx")).to_excel(
        w, sheet_name="17_vs_Baseline", index=False)
    pd.DataFrame({
        "section": ["CA Tanimi", "IP Formulasyonu", "12 Senaryo", "Bulgular", "Sinirliliklar"],
        "content": [
            "CA9a: Plain IP + spatial constraint (her mahallede min 1 site, 15 konutlu mahalle)",
