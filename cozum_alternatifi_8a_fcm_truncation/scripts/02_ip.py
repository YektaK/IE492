"""
CA8a - IP (3 AHP x 2 mode = 6 senaryo)
Truncated mu matrisini kullanir.

Cikti:
  - results/selection_Baseline_hard.xlsx
  - results/selection_Baseline_soft.xlsx
  - results/selection_DamageFocused_hard.xlsx
  - results/selection_DamageFocused_soft.xlsx
  - results/selection_InfrastructureFocused_hard.xlsx
  - results/selection_InfrastructureFocused_soft.xlsx
  - results/selection_sensitivity.xlsx
"""
import os
import math
import unicodedata
...
def load_inputs():
    mu_aday = pd.read_excel(os.path.join(DATA, "mu_aday_truncated.xlsx"))
    mu_mev = pd.read_excel(os.path.join(DATA, "mu_mevcut_truncated.xlsx"))
...
def solve_ip(mu_aday, mu_mev, mahalleler, risk_map, scenario_name, mode, threshold=COVERAGE_THRESHOLD):
    n = len(mu_aday)
    prob = pulp.LpProblem(f"CA8a_{scenario_name}_{mode}", pulp.LpMaximize)
    ...
    return {
        ...
        "Z": z_val,
        "selected": selected,
        "coverage_per_mahalle": cov_per_mh,
    }
...
def main():
    ...
    for scen in scenarios:
        for mode in modes:
            ...
            sel["mode"] = r["mode"]
            sel["Z"] = r["Z"]
            sel_path = os.path.join(RES, f"selection_{r['scenario']}_{mode}.xlsx")
            sel.to_excel(sel_path, index=False)
