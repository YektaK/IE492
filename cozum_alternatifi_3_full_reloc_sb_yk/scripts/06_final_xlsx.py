...
if __name__ == "__main__":
    print("=" * 60)
    print("STEP 6: CA3 Final xlsx (18 sayfa)")
    print("=" * 60)

    sheets = {}
    readme = ("# Sultanbeyli - Cozum Alternatifi 3: Tam Yer Degisikligi (SB+YK)\n"
    ...
    sens = pd.read_excel(RES / "selection_sensitivity_CA3.xlsx")
    sheets["KPI_Summary"] = pd.DataFrame({
        "metrik": ["Secilen konteyner sayisi", "Senaryo sayisi", "Sigma (m)",
                    "Coverage threshold", "Ortalama Z (CA3)",
    ...
    sel = pd.read_excel(RES / "selection_Baseline_hard_CA3.xlsx", sheet_name="Selected_20")
    sheets["Selected_20"] = sel
    sheets["AHP_Weights"] = pd.read_excel(RES / "ahp_weights_CA3.xlsx")
    top = pd.read_excel(RES / "topsis_sonuclar_CA3.xlsx").nlargest(20, "CC_Baseline")
    sheets["TOPSIS_Top20"] = top
    sheets["Sensitivity"] = sens
    cov = pd.read_excel(RES / "selection_Baseline_hard_CA3.xlsx", sheet_name="Coverage_per_Mahalle")
    ...
    sheets["Mu_Pool_152"] = pd.read_excel(RES / "mu_pool_CA3.xlsx")
    sheets["Centroids"] = pd.read_excel(RES / "mahalle_centroids_CA3.xlsx")
    sheets["Criteria_Matrix"] = pd.read_excel(RES / "criteria_matrix_CA3.xlsx")
    sheets["Pool_152"] = pd.read_excel(RES / "pool_152_CA3.xlsx")
    sheets["Road_Closure"] = pd.read_excel(CA3 / "data" / "mahalle_data_CA3.xlsx")
    sheets["P_Access"] = pd.read_excel(RES / "p_access_per_site_CA3.xlsx")

    out_path = FIN / "Sultanbeyli_Final_Results_CA3.xlsx"
    ...
    md.append("## Sultanbeyli Konteyner Optimizasyonu - Tam Yer Degisikligi (SB+YK)")
    md.append("")
    ...
    md.append(f"| Metrik | Orijinal (8) | CA2 (8) | CA3 (20) |")
    md.append(f"|--------|--------------|---------|----------|")
