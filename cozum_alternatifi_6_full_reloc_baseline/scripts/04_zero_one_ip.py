# -*- coding: utf-8 -*-
"""
04_zero_one_ip_CA6.py
==========================================================
Cozum Alternatifi 3: 0-1 IP solver (full relocation).
- 152 havuz, 20 secim (12 mevcut + 8 yeni yerine 20 yeni yer)
- baseline etkisi: C4 = barinma ihtiyaci, P_access carpani

Max Z = Sum_i R_i * [ 0  +  Sum_j mu(i,j) * P_access(j) * X_j ]
                                  ^^^^^^^^^^^^^^^^^^^^^^^^^^^
                                  mu_mevcut = 0 (artik sabit konteyner yok)
3 AHP x 2 mod = 6 senaryo.
==========================================================
Ciktilar:
  - results/selection_<scenario>_<mode>_CA6.xlsx
  - results/selection_sensitivity_CA6.xlsx
  - results/p_access_per_site_CA6.xlsx
"""
import unicodedata
import numpy as np
import pandas as pd
...
def load_inputs():
    mu_pool = pd.read_excel(RES / "mu_pool_CA6.xlsx")
    mu_mev_zero = pd.read_excel(RES / "mu_mevcut_zero_CA6.xlsx")
    skip = {"_id", "S_No", "Mahalle", "container_no", "mahalle", "_TOTAL_mu", "_kaynak"}
    mahalleler = [c for c in mu_pool.columns if c not in skip]
    ...
    crit = pd.read_excel(RES / "topsis_sonuclar_CA6.xlsx")
    return mu_pool, mu_mev_zero, mahalleler, risk_map, crit

def build_p_access(mu_pool_df, mahalleler):
    """CA6 (baseline): YK etkisi YOK -> P_access = 1.0 (tüm siteler eşit erişilebilir)."""
    ...
    p_access = []
    site_mh = []
    for _, r in mu_pool_df.iterrows():
        mus = {mh: float(r[mh]) for mh in mahalleler}
        dom = max(mus, key=mus.get)
        # CA6: P_access = 1.0 (yol kapanma etkisi devre disi)
        p = 1.0
        p_access.append(p)
        site_mh.append(dom)
    return p_access, site_mh

def solve_ip(mu_pool_df, mu_mev_df, mahalleler, risk_map, p_access_arr,
             scenario_name, mode, lambda_penalty=10.0):
    n = len(mu_pool_df)
    prob = pulp.LpProblem(f"CA6_Sultanbeyli_{scenario_name}_{mode}", pulp.LpMaximize)
    ...
    total_coverage = {}
    for mh in mahalleler:
        new_part = pulp.lpSum(float(mu_pool_df.iloc[j][mh]) * p_access_arr[j] * X[j]
                              for j in range(n))
...
def build_report(result, mu_pool_df, crit_df, p_access_arr, site_mh, out_path):
    sel = result["selected_idx"]
    base = result["base_coverage"]
    mahalleler = [c for c in mu_pool_df.columns
                  if c not in {"_id", "S_No", "Mahalle", "container_no", "mahalle",
                               "_TOTAL_mu", "_kaynak"}]
    R = result["R"]
    sel_rows = []
    for j in sel:
        c = crit_df[crit_df["S_No"] == int(mu_pool_df.iloc[j]["S_No"])]
        if len(c) == 0: continue
        c = c.iloc[0]
        sel_rows.append({
            "S_No": int(c["S_No"]),
            "_kaynak": mu_pool_df.iloc[j]["_kaynak"],
            "Alan_Adi": c["Alan_Adi"],
            "Mahalle": c["Mahalle"],
            "Enlem": c["Enlem"],
            "Boylam": c["Boylam"],
            ...
            "FCM_total_mu": float(mu_pool_df.iloc[j]["_TOTAL_mu"]),
        })
    df_sel = pd.DataFrame(sel_rows)
    cov_rows = []
    for mh in mahalleler:
        base_v = base.get(mh, 0.0)
        new_v = sum(float(mu_pool_df.iloc[j][mh]) * p_access_arr[j] for j in sel)
        total = base_v + new_v
        R_v = R.get(mh, 1.0)
        ...
        cov_rows.append({
            ...
            "R_risk_weight": R_v,
            "baseline_coverage_0": base_v,
            "new_coverage_20_weighted": new_v,
    ...
    kpi = {
        ...
        "sum_baseline_coverage": float(df_cov["baseline_coverage_0"].sum()),
        "coverage_uplift_pct": float("inf") if df_cov["baseline_coverage_0"].sum() == 0 else
                              float((df_cov["total_coverage_20_weighted"].sum() - df_cov["baseline_coverage_0"].sum()) / max(df_cov["baseline_coverage_0"].sum(), 1e-9) * 100),
    }
    df_kpi = pd.DataFrame([kpi])
    with pd.ExcelWriter(out_path) as w:
        df_sel.to_excel(w, sheet_name="Selected_20", index=False)
...
if __name__ == "__main__":
    print("=" * 60)
    print(f"STEP 4: 0-1 IP - Full Relocation (20 secim) - CA6 (baseline)")
    print("=" * 60)
    mu_pool, mu_mev, mahalleler, risk_map, crit = load_inputs()
    p_access, site_mh = build_p_access(mu_pool, mahalleler)
    print(f"P_access araligi: {min(p_access):.4f} - {max(p_access):.4f}  (ortalama {np.mean(p_access):.4f})")
    p_access_df = pd.DataFrame({
        "S_No": mu_pool["S_No"], "Mahalle_display": mu_pool["Mahalle"],
        "_kaynak": mu_pool["_kaynak"],
        "dominant_mahalle_norm": site_mh, "P_access": p_access
    })
    p_access_df.to_excel(RES / "p_access_per_site_CA6.xlsx", index=False)
    print(f"[OK] p_access_per_site_CA6.xlsx")

    results = []
    for sc in ["Baseline", "DamageFocused", "InfrastructureFocused"]:
        for mode in ["hard", "soft"]:
            print(f"\nSolving: scenario={sc}, mode={mode}")
            res = solve_ip(mu_pool, mu_mev, mahalleler, risk_map, p_access,
                           sc, mode, lambda_penalty=10.0)
            out_path = RES / f"selection_{sc}_{mode}_CA6.xlsx"
            df_sel, df_cov, df_kpi = build_report(res, mu_pool, crit, p_access,
                                                  site_mh, out_path)
    ...
    df_res.to_excel(RES / "selection_sensitivity_CA6.xlsx", index=False)
    print(f"\n[OK] selection_sensitivity_CA6.xlsx")
    print()
    print("--- CA6 Sensitivity Summary ---")
