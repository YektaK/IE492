"""
CA8b - IP (3 AHP x 2 mode = 6 senaryo)
Adaptive sigma mu matrisini kullanir.
"""
import os
import unicodedata
import numpy as np
...
def load_inputs():
    mu_aday = pd.read_excel(os.path.join(DATA, "mu_aday_adaptive.xlsx"))
    mu_mev = pd.read_excel(os.path.join(DATA, "mu_mevcut_adaptive.xlsx"))
...
def solve_ip(mu_aday, mu_mev, mahalleler, risk_map, scenario_name, mode, threshold=COVERAGE_THRESHOLD):
    n = len(mu_aday)
    prob = pulp.LpProblem(f"CA8b_{scenario_name}_{mode}", pulp.LpMaximize)
