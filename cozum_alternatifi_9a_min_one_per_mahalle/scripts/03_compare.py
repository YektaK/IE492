"""
03_compare.py - CA9a vs baseline
"""
import os
import pandas as pd
import numpy as np
...
df.to_excel(os.path.join(COMPARISON_DIR, "CA9a_vs_baseline.xlsx"), index=False)
print(f"[OK] CA9a_vs_baseline.xlsx ({len(df)} satir)")

with open(os.path.join(COMPARISON_DIR, "comparison_report.md"), "w", encoding="utf-8") as f:
    f.write("# CA9a vs Baseline Comparison\n\n")
