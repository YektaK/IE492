...
05_compare_CA5.py
==========================================================
Cozum Alternatifi 3: CA5 (full relocation + YK only) ile
orijinal 8-site secimi (ve CA2) arasindaki farki analiz eder.
==========================================================
Ciktilar:
  - comparison/compare_overview_CA5.png
  - comparison/compare_summary_CA5.xlsx
  - comparison/comparison_report_CA5.md
"""
import unicodedata
import numpy as np
import pandas as pd
...
def main():
    ...
    sel_ca3 = pd.read_excel(RES_CA5 / "selection_Baseline_hard_CA5.xlsx", sheet_name="Selected_20")
    sens_orig = pd.read_excel(RES_ORIG / "selection_sensitivity.xlsx")
    sens_ca2 = pd.read_excel(RES_CA2 / "selection_sensitivity_CA2.xlsx")
    sens_ca3 = pd.read_excel(RES_CA5 / "selection_sensitivity_CA5.xlsx")
    ...
    print(f"Orijinal (8)  vs CA5 (20): ortak = {len(common_orig_ca3)}")
    print(f"CA2 (8)       vs CA5 (20): ortak = {len(common_ca2_ca3)}")
    print(f"CA5'un icindeki 12 mevcut: {sum(1 for _, r in sel_ca3.iterrows() if r['_kaynak'] == 'mevcut_12')}")
    ...
    # Mahalle coverage
    cov_ca3 = pd.read_excel(RES_CA5 / "selection_Baseline_hard_CA5.xlsx", sheet_name="Coverage_per_Mahalle")
    ...
    cov_join = cov_orig[["_mh", "mahalle", "R_risk_weight"]].rename(
        ...
        cov_ca3[["_mh", "total_coverage_20_weighted"]].rename(
            columns={"total_coverage_20_weighted": "coverage_CA5"}),
        on="_mh", how="outer"
    ).fillna(0)
    cov_join = cov_join.merge(road[["_mh", "P_road_open"]].drop_duplicates("_mh"), on="_mh", how="left")
    print("\n--- Mahalle coverage (CA5) ---")
    print(cov_join[["mahalle", "P_road_open", "coverage_CA5"]].to_string(index=False))
    ...
    sens_join = sens_join.merge(sens_ca3[["scenario", "mode", "objective"]].rename(columns={"objective": "Z_CA5"}),
                                 on=["scenario", "mode"], how="outer")
    print("\n--- Sensitivity (3 senaryo) ---")
    print(sens_join.to_string(index=False))

    # Save
    with pd.ExcelWriter(CMP / "compare_summary_CA5.xlsx") as w:
        sel_ca3.to_excel(w, sheet_name="CA5_20_sites", index=False)
        cov_join.to_excel(w, sheet_name="Mahalle_coverage", index=False)
        sens_join.to_excel(w, sheet_name="Sensitivity_3way", index=False)
    print(f"\n[OK] compare_summary_CA5.xlsx")
    ...
    ax.bar(x + w, sens_join["Z_CA5"], w, label="CA5 (20)", color="seagreen")
    ax.set_xticks(x)
    ax.set_xticklabels(scens, rotation=30, ha="right", fontsize=8)
    ax.set_ylabel("Objektif Degeri (Z)", fontsize=10)
    ax.set_title("6 Senaryo: Orijinal vs CA2 vs CA5", fontsize=11, fontweight="bold")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)

    ax = axes[1]
    cov_sorted = cov_join.sort_values("coverage_CA5", ascending=True)
    ax.barh(cov_sorted["mahalle"], cov_sorted["coverage_CA5"],
            color=cov_sorted["P_road_open"].apply(
                lambda p: "green" if p >= 0.98 else ("orange" if p >= 0.94 else "red")))
    ax.set_xlabel("Coverage (CA5, P_access weighted)", fontsize=10)
    ax.set_title("Mahalle Coverage - CA5", fontsize=11, fontweight="bold")
    ax.grid(axis="x", alpha=0.3)
    plt.tight_layout()
    plt.savefig(CMP / "compare_overview_CA5.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[OK] compare_overview_CA5.png")

    # Markdown
    md = []
    md.append("# Cozum Alternatifi 3 - Tam Yer Degisikligi (YK only)")
    md.append("## Orijinal vs CA2 vs CA5 Karsilastirma")
    md.append("")
    md.append("## 1. Yontem")
    ...
    md.append(f"- CA5 (20):     {sorted(ca3_set)}")
    md.append(f"- CA5 icindeki mevcut sayisi: {sum(1 for _, r in sel_ca3.iterrows() if r['_kaynak'] == 'mevcut_12')}")
    md.append(f"- CA5 ∩ Orijinal: {sorted(common_orig_ca3)}")
    md.append(f"- CA5 ∩ CA2:      {sorted(common_ca2_ca3)}")
    md.append("")
    md.append("## 3. CA5 20 Site Detay")
    md.append("")
    ...
    md.append("")
    md.append("| Senaryo | Mod | Z Orijinal | Z CA2 | Z CA5 |")
    md.append("|---------|-----|-----------|-------|-------|")
    for _, r in sens_join.iterrows():
        md.append(f"| {r['scenario']} | {r['mode']} | {r['Z_orig']:.2f} | {r['Z_CA2']:.2f} | {r['Z_CA5']:.2f} |")
    md.append("")
    md.append("## 5. Sonuc")
    md.append(f"20 konteyner secimde Z = {sens_join['Z_CA5'].mean():.2f} (orijinal 8-site Z = 935.62'in ~%{sens_join['Z_CA5'].mean()/935.62*100:.0f}'i). "
              "12+8 -> 20 konteyner gecisiyle Z beklenen sekilde artti (daha fazla konteyner = daha fazla coverage).")
    (CMP / "comparison_report_CA5.md").write_text("\n".join(md), encoding="utf-8")
    print(f"[OK] comparison_report_CA5.md")
