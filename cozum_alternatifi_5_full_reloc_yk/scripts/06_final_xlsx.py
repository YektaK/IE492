...
if __name__ == "__main__":
    print("=" * 60)
    print("STEP 6: CA5 Final xlsx (18 sayfa)")
    print("=" * 60)

    sheets = {}
    readme = ("# Sultanbeyli - Cozum Alternatifi 3: Tam Yer Degisikligi (YK only)\n"
    ...
    sens = pd.read_excel(RES / "selection_sensitivity_CA5.xlsx")
    sheets["KPI_Summary"] = pd.DataFrame({
        "metrik": ["Secilen konteyner sayisi", "Senaryo sayisi", "Sigma (m)",
                    "Coverage threshold", "Ortalama Z (CA5)",
    ...
    sel = pd.read_excel(RES / "selection_Baseline_hard_CA5.xlsx", sheet_name="Selected_20")
    sheets["Selected_20"] = sel
    sheets["AHP_Weights"] = pd.read_excel(RES / "ahp_weights_CA5.xlsx")
    top = pd.read_excel(RES / "topsis_sonuclar_CA5.xlsx").nlargest(20, "CC_Baseline")
    sheets["TOPSIS_Top20"] = top
    sheets["Sensitivity"] = sens
    cov = pd.read_excel(RES / "selection_Baseline_hard_CA5.xlsx", sheet_name="Coverage_per_Mahalle")
    ...
    sheets["Mu_Pool_152"] = pd.read_excel(RES / "mu_pool_CA5.xlsx")
    sheets["Centroids"] = pd.read_excel(RES / "mahalle_centroids_CA5.xlsx")
    sheets["Criteria_Matrix"] = pd.read_excel(RES / "criteria_matrix_CA5.xlsx")
    sheets["Pool_152"] = pd.read_excel(RES / "pool_152_CA5.xlsx")
    sheets["Road_Closure"] = pd.read_excel(CA5 / "data" / "mahalle_data_CA5.xlsx")
    sheets["P_Access"] = pd.read_excel(RES / "p_access_per_site_CA5.xlsx")

    out_path = FIN / "Sultanbeyli_Final_Results_CA5.xlsx"
    ...
    md.append("## Sultanbeyli Konteyner Optimizasyonu - Tam Yer Degisikligi (YK only)")
    md.append("")
    md.append("**Tarih:** 2026  \n**Pipeline:** AHP + TOPSIS + FCM + 0-1 IP  \n"
              "**Orijinal:** `D:/IE492/output/`  \n**CA5:** `D:/IE492/cozum_alternatifi_5_full_reloc_yk/`  \n")
    md.append("## 1. Problem")
    ...
    md.append(f"| Metrik | Orijinal (8) | CA2 (8) | CA5 (20) |")
    md.append(f"|--------|--------------|---------|----------|")
